...
public class DateIsAfterProvidedDateValidator
    implements ConstraintValidator<DateIsAfterProvidedDate, Object> {
    ...
    try {
      ...
      if (!(baseFieldValue instanceof Date)) {
        throw new NotDateException(
            "field with name " + this.baseProvidedDateFieldName + " is not type of java.util.Date");
      }
      if (!(testFieldValue instanceof Date)) {
        throw new NotDateException(
            "field with name " + this.testDateFieldName + " is not type of java.util.Date");
      }
      ...
      String errormessage =
          "Can not extract field for class: "
